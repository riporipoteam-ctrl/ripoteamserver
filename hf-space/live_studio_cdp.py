...
class LiveStudioCDP:
    ...
    def go_live(self) -> dict[str, Any]:
        with self.lock:
            ...
            targets = _targets()
            if not state.get("live_studio_running") and not targets:
                ...
                while time.time() < deadline:
                    ...
                    state = self.wine_runner.status()
                    targets = _targets()
                    if state.get("live_studio_running") or targets:
                        break
                    if state.get("phase") == "wine-failed":
                        raise RuntimeError(str(state.get("last_error") or "LIVE Studio failed to start."))
                if not state.get("live_studio_running") and not targets:
                    raise RuntimeError("LIVE Studio did not expose either a visible UI or a localhost Electron control target in time.")

            cdp_deadline = time.time() + 45
            while time.time() < cdp_deadline and not _targets():
                time.sleep(1)
            if not _targets():
                raise RuntimeError("LIVE Studio process is alive, but its localhost Electron control target did not start.")
            ...
            if not clicked:
                ...
                raise RuntimeError("LIVE Studio is running headlessly but no Go LIVE button is exposed yet. Actions: " + ", ".join(labels[:12]))
            ...
            return {
                "ok": True,
                "clicked": clicked,
                "message": "LIVE Studio Go LIVE action was sent through the localhost Electron control target; Ripo Bot AI host is starting.",
            }
