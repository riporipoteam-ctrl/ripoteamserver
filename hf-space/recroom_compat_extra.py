...
def install_recroom_extra_routes(app: Any, gateway: RecRoomGateway) -> None:
    ...
    These DTO categories follow the recovered server map: list endpoints return
    lists, resource/config endpoints return objects, and mutations acknowledge
    with objects. Unknown calls remain 404 for diagnostics.
    """
    ...
    # Recovered list-style endpoints.
    list_gets = {
    ...
    # Recovered object/resource-style endpoints.
    object_gets: dict[str, tuple[str, dict[str, Any]]] = {
        ...
        "/econ/roomOffer/purchaseCounts": ("rr2022_purchase_counts", {}),
    }
    ...
    # Room browsing endpoints are static paths. recroom_compat.py already added
    # /Room_server/rooms/{room_id}; add these and then move dynamic room routes to
    # the end so Starlette cannot match "hot" or "ownedby" as room_id first.
    room_lists = {
    ...
    # Parameterized economy endpoints.
