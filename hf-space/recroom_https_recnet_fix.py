...
_PATCH_REVISION = "may2022-loopback-bootstrap-v5"
_TRANSPORT_REVISION = "loopback-http-bootstrap-local-http-v3"
...
# Build 8751857's bootstrap is redirected straight into its per-player loopback
# proxy. No external HTTPS nameserver is involved; the proxy returns the exact
# May-2022 nameserver contract and keeps all authenticated RecNet traffic local.
...
def _bootstrap_url(self: RecRoomWinePool, ip: str) -> str:
    ...
    return f"http://{address}:81{nameserver_fix.LOCAL_NAMESERVER_PATH}"
...
def _relocate_bootstrap_literal(path: Path, target: str) -> int:
    ...
    nameserver_fix._atomic_replace(path, bytes(data), "loopbackbootstrap")
    return 1


def _patch_client_loopback(self: RecRoomWinePool, root: Path, local_base: str) -> int:
...
def install_public_bootstrap_route(application: Any) -> None:
    """Retain the public bootstrap route for diagnostics; v5 clients do not use it."""
...
def _capability_loopback(self: RecRoomWinePool) -> dict[str, Any]:
    ...
    payload["recNetBootstrapPath"] = nameserver_fix.LOCAL_NAMESERVER_PATH
    payload["recNetBootstrapScheme"] = "http"
...
nameserver_fix._patch_client = _patch_client_loopback
RecRoomWinePool._patch_client = _patch_client_loopback  # type: ignore[method-assign]
RecRoomWinePool._start_proxy = black_viewport._start_proxy_traced  # type: ignore[method-assign]
RecRoomWinePool.capability = _capability_loopback  # type: ignore[method-assign]
print(f"Rec Room loopback RecNet bootstrap patch loaded: {_PATCH_REVISION} / {_TRANSPORT_REVISION}")
