...
_PATCH_REVISION = "may2022-static-bootstrap-v4"
_TRANSPORT_REVISION = "github-raw-https-bootstrap-local-http-v2"
_PUBLIC_BOOTSTRAP_PATH = "/api/recroom-bootstrap/ns"
_STATIC_BOOTSTRAP_COMMIT = "6cfb4727f40037cf8ed9b6e29d80231699ae3be1"
_STATIC_BOOTSTRAP_ROOT = (
    "https://raw.githubusercontent.com/riporipoteam-ctrl/recroomfluxgame/"
    f"{_STATIC_BOOTSTRAP_COMMIT}/bootstrap"
)
_STATIC_BOOTSTRAP_IPS = {"127.0.0.1", "127.0.0.2"}
...
# Only the tiny nameserver bootstrap leaves the sandbox over HTTPS. Its response
# contains loopback-only service URLs; all authenticated RecNet traffic stays on
# the per-player local proxy where the RipoTeamServer session token is injected.
...
def _loopback_ip(local_base: str) -> str:
    ...
    ip = str(address)
    if ip not in _STATIC_BOOTSTRAP_IPS:
        raise RuntimeError(
            f"Rec Room static bootstrap is not provisioned for sandbox address {ip!r}."
        )
    return ip
...
def _bootstrap_url(self: RecRoomWinePool, ip: str) -> str:
    del self
    if ip not in _STATIC_BOOTSTRAP_IPS:
        raise RuntimeError(f"No pinned RecNet bootstrap exists for {ip!r}.")
    return f"{_STATIC_BOOTSTRAP_ROOT}/ns-{ip}.json"
...
def install_public_bootstrap_route(application: Any) -> None:
    """Keep the HF bootstrap route available for diagnostics; v4 clients use pinned GitHub JSON."""
...
def _capability_trusted(self: RecRoomWinePool) -> dict[str, Any]:
    ...
    payload["recNetBootstrapHost"] = "raw.githubusercontent.com"
    payload["recNetBootstrapBase"] = _STATIC_BOOTSTRAP_ROOT
    payload["recNetBootstrapPinnedCommit"] = _STATIC_BOOTSTRAP_COMMIT
    payload["recNetBootstrapSlots"] = sorted(_STATIC_BOOTSTRAP_IPS)
...
print(f"Rec Room static RecNet bootstrap patch loaded: {_PATCH_REVISION} / {_TRANSPORT_REVISION}")
