...
$targets = @(
  [pscustomobject]@{
    Name = "recroom-agent.ps1"
    Url = "https://raw.githubusercontent.com/$hostRepo/$hostRef/windows-live-agent/recroom-agent.ps1"
    Destination = Join-Path $PSScriptRoot "recroom-agent.ps1"
    Kind = "powershell"
  },
  [pscustomobject]@{
    Name = "recroom-capture-agent.ps1"
    Url = "https://raw.githubusercontent.com/$hostRepo/$hostRef/windows-live-agent/recroom-capture-agent.ps1"
    Destination = Join-Path $PSScriptRoot "recroom-capture-agent.ps1"
    Kind = "powershell"
  },
  [pscustomobject]@{
    Name = "host-proxy.mjs"
    Url = "https://raw.githubusercontent.com/$toolsRepo/$toolsRef/scripts/host-proxy.mjs"
    Destination = Join-Path $toolDir "host-proxy.mjs"
    Kind = "node"
  },
  [pscustomobject]@{
    Name = "verify-client.mjs"
    Url = "https://raw.githubusercontent.com/$toolsRepo/$toolsRef/scripts/verify-client.mjs"
    Destination = Join-Path $toolDir "verify-client.mjs"
    Kind = "node"
  },
  [pscustomobject]@{
    Name = "scan-client-urls.mjs"
    Url = "https://raw.githubusercontent.com/$toolsRepo/$toolsRef/scripts/scan-client-urls.mjs"
    Destination = Join-Path $toolDir "scan-client-urls.mjs"
    Kind = "node"
  },
  [pscustomobject]@{
    Name = "patch-client-urls.mjs"
    Url = "https://raw.githubusercontent.com/$toolsRepo/$toolsRef/scripts/patch-client-urls.mjs"
    Destination = Join-Path $toolDir "patch-client-urls.mjs"
    Kind = "node"
  }
)
...
function Test-NodeSyntax([string]$Path) {
  ...
  if (-not $node) {
    throw "Node.js 20+ is required to validate and run the Rec Room host proxy tools."
  }
...
try {
  foreach ($target in $targets) {
    ...
    if (Test-Path $target.Destination) {
      $backup = "$($target.Destination).update-backup"
